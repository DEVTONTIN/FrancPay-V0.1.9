create table if not exists "ApplicationFeeLedger" (
...
create table if not exists "ApplicationFeeBalance" (
  "id" integer primary key default 1,
  "balanceFre" numeric(18,2) not null default 0,
  "updatedAt" timestamptz not null default now()
);

insert into "ApplicationFeeBalance" ("id","balanceFre")
values (1, 0)
on conflict ("id") do nothing;

create or replace function public.record_application_fee(
  p_amount numeric,
...
begin
  ...
  insert into "ApplicationFeeLedger" ("context","reference","amountFre","metadata")
  values (coalesce(nullif(p_context,''), 'general'), nullif(p_reference,''), p_amount, coalesce(p_metadata, '{}'::jsonb));

  update "ApplicationFeeBalance"
     set "balanceFre" = "balanceFre" + p_amount,
         "updatedAt" = now()
   where "id" = 1;
end;
$$;
...
begin
  ...
  perform public.record_application_fee(
    ...
    'staking_lock',
    v_product."code",
    jsonb_build_object('positionId', v_position_id)
  );
